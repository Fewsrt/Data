...
public class Infix {
    public static void main(String[] args) {
        ...
        String[] tokens = input.nextLine().split(" "); // read input and store it as array of strings delimited by
                                                       // spaces
        for (int i = 0; i < tokens.length; i++) // take one token at a time
        {
            if (tokens[i].equals("("))
                ; // just ignore left paranthesis
            ...
            else if (tokens[i].equals(")")) // if token is ")", Pop, evaluate, and push result
            {
                ...
                if (op.equals("+"))
                    v = vals.pop() + v; // perform corresponding operation by poping another value and store it in v
                                        // itself
                else if (op.equals("-"))
                    v = vals.pop() - v;
                else if (op.equals("*"))
                    ...
                    v = Math.pow(vals.pop(), v); // to calculate exponent(^)
                vals.push(v); // push the result into Values stack
            } else
                vals.push(Double.parseDouble(tokens[i])); // if none of above if applies, the token should be a number.
                                                          // Push it into Values stack
        }
